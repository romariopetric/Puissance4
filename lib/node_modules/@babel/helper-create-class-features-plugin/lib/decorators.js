...
function transformClass(path, state, constantSuper, ignoreFunctionLength, className, propertyVisitor, version) {
    ...
    for (const element of body) {
      ...
      if (hasDecorators) {
        ...
        if (kind === ACCESSOR) {
          ...
          const newId = generateClassPrivateUid();
          const newFieldInitId = generateLetUidIdentifier(scopeParent, "init_".concat(name));
          const newValue = _core.types.callExpression(_core.types.cloneNode(newFieldInitId), params);
          const newField = generateClassProperty(newId, newValue, isStatic);
          const [newPath] = element.replaceWith(newField);
          if (isPrivate) {
            privateMethods = extractProxyAccessorsFor(newId, version);
            const getId = generateLetUidIdentifier(scopeParent, "get_".concat(name));
            const setId = generateLetUidIdentifier(scopeParent, "set_".concat(name));
          ...
          const initId = generateLetUidIdentifier(scopeParent, "init_".concat(name));
          ...
          const callId = generateLetUidIdentifier(scopeParent, "call_".concat(name));
          locals = [callId];
      ...
      if (hasDecorators && version === "2023-11") {
        if (kind === FIELD || kind === ACCESSOR) {
          const initExtraId = generateLetUidIdentifier(scopeParent, "init_extra_".concat(name));
  ...
  if (classDecorators) {
    ...
    if (statics.length > 0 || staticFieldInitializerExpressions.length > 0) {
      const staticsClass = _core.template.expression.ast(_templateObject5 || (_templateObject5 = _taggedTemplateLiteral(["\n        class extends ", " {}\n      "])), state.addHelper("identity"));
