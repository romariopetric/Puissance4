...
function buildPrivateStaticFieldInitSpec(prop, privateNamesMap, noUninitializedPrivateFieldAccess) {
  ...
  const value = noUninitializedPrivateFieldAccess ? prop.node.value : _core.template.expression.ast(_templateObject10 || (_templateObject10 = _taggedTemplateLiteral(["{\n        _: ", "\n      }"])), prop.node.value || _core.types.buildUndefinedNode());
...
function buildPrivateMethodDeclaration(file, prop, privateNamesMap) {
  let privateFieldsAsSymbolsOrProperties = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
  const privateName = privateNamesMap.get(prop.node.key.id.name);
  const {
    id,
    methodId,
