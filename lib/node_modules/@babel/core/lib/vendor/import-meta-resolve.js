...
function formatList(array) {
  let type = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'and';
...
codes.ERR_MODULE_NOT_FOUND = createError('ERR_MODULE_NOT_FOUND', function (path, base) {
  let exactUrl = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
...
function read(jsonPath, _ref) {
  ...
  if (string !== undefined) {
    ...
    try {
      ...
      const cause = error_;
      const error = new ERR_INVALID_PACKAGE_CONFIG$1(jsonPath, (base ? "\"".concat(specifier, "\" from ") : '') + (0, _url().fileURLToPath)(base || specifier), cause.message);
      error.cause = cause;
      throw error;
    }
    result.exists = true;
...
function throwInvalidSubpath(request, match, packageJsonUrl, internal, base) {
  const reason = "request is not a valid match in pattern \"".concat(match, "\" for the \"").concat(internal ? 'imports' : 'exports', "\" resolution of ").concat((0, _url().fileURLToPath)(packageJsonUrl));
