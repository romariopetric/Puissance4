...
function tryRequireResolve(id, dirname) {
  try {
    if (dirname) {
      return {
        error: null,
        value: (((v, w) => (v = v.split("."), w = w.split("."), +v[0] > +w[0] || v[0] == w[0] && +v[1] >= +w[1]))(process.versions.node, "8.9") ? require.resolve : function (r, _ref) {
          let {
            paths: [b]
          } = _ref;
...
function* requireModule(type, loader, name) {
  ...
  try {
    ...
    {
      return yield* (0, _moduleTypes.default)(name, loader, "You appear to be using a native ECMAScript module ".concat(type, ", ") + "which is only supported when running Babel asynchronously " + "or when using the Node.js `--experimental-require-module` flag.", "You appear to be using a ".concat(type, " that contains top-level await, ") + "which is only supported when running Babel asynchronously.", true);
    }
  } catch (err) {
